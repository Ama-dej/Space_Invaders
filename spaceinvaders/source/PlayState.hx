...
class PlayState extends BarricadeCreationState
{
	...
	override public function create()
	{
		...
		Player.health = 3;
		coolCooldownShit = friendlyCooldown;
		#if debug
		Player.health = 999;
		coolCooldownShit = -1;
		#end
	...
	override public function update(elapsed:Float)
	{
		...
		if ((FlxG.mouse.justPressed || FlxG.keys.anyJustPressed([ENTER, SPACE])) && friendlyCooldown < 0) {
			...
			friendlyCooldown = coolCooldownShit;
		}
		friendlyCooldown -= 1;
		recentlyGotShot -= 1;
	}
	...
	function enemyTooClose() {
		if (EnemyArray[EnemyArray.length - 1].y > Player.y - 150) {
			trace("dead");
			var dedLol:Died = new Died(EnemyArray);
			openSubState(dedLol);
		}
	}
...
class Died extends FlxSubState {
	...
	var enemyLength:Int = 0;
	public function new(enemyarray:Array<FlxSprite>) {
		super();
	...
	override public function create() {
		super.create();
		var youDiedText:FlxText = new FlxText();
		if (enemyLength == 0) {
			youDiedText.text = "You Win";
		} else
	...
	override public function update(elapsed:Float) {
		super.update(elapsed);
		if (FlxG.keys.anyJustPressed([ESCAPE, ENTER, TAB, SPACE])) {
			FlxG.resetGame();
		}
	}
}